	...
	public static void init() {
		if (Guilds.plugin.getConfig().getString("Storage.engine").equalsIgnoreCase("mysql")) {
			...
			if (!sql.tableExists("guilds_players")) {
				...
				String query = "CREATE TABLE `guilds_players` (" + "`uuid` varchar(36) NOT NULL," + "`name` varchar(255) NOT NULL," + "`previous_guilds` varchar(255)," + "`current_guild` varchar(255)," + " PRIMARY KEY (uuid, name));";
				sql.modifyQuery(query, false);
			}
		} else {
